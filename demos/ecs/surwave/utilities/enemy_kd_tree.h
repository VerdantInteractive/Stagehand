...
namespace enemy_kd_tree {
    ...
    class KdTree2D {
      public:
        ...
        [[nodiscard]] bool empty() const { return root_index < 0; }

        template <typename PositionAccessor> void build(std::int32_t entity_count, const PositionAccessor &position_accessor) {
            clear();
            if (entity_count <= 0) {
                return;
            }
        ...
        template <typename PositionAccessor> void refresh_points(std::int32_t entity_count, const PositionAccessor &position_accessor) {
            if (entity_count <= 0) {
                return;
            }
        ...
        void radius_query(const godot::Vector2 &origin, float radius_squared, const Visitor &visitor, std::int32_t max_results = -1) const {
      ...
      private:
        template <typename PositionAccessor> class AxisComparator {
          public:
        ...
        std::int32_t build_recursive(std::int32_t start, std::int32_t end, std::int32_t depth, const PositionAccessor &position_accessor) {
            ...
            std::nth_element(permutation.begin() + start, permutation.begin() + median, permutation.begin() + end, comparator);
        ...
        template <typename Visitor>
        bool
        query_recursive(std::int32_t node_index, const godot::Vector2 &origin, float radius_squared, const Visitor &visitor, std::int32_t &remaining) const {
            if (node_index < 0) {
                return true;
            }
            ...
            const float axis_delta = node.axis == 0 ? origin.x - node.point.x : origin.y - node.point.y;
            ...
            const float axis_distance_squared = axis_delta * axis_delta;
