...
Through macros, boilerplate-free component definition and registration of components is possible.
...
### `INT64(Name, ...)` - 64-bit signed integer
```cpp
...
### `UINT64(Name, ...)` - 64-bit unsigned integer
```cpp
...
Empty marker component for entity classification and filtering. Tags carry no data.
```cpp
...
### `POINTER(Name, Type, [DefaultValue])`
...
### `VECTOR(Name, ElementType, [Initialiser])`
...
### `ARRAY(Name, ElementType, Count, [Initialiser])`
...
### `GODOT_VARIANT(Name, Type, [DefaultValue])`

Wraps Godot's built-in Variant types as components. Supports both struct-based (Plain Old Data) types and class-based (handle) types.
