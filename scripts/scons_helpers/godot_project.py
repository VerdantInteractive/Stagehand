...
def check_and_setup_project_file_structure(relative_path):
    ...
    if not os.path.isfile(project_file_path):
        ...
        # Ensure the project ecs subdirectory exists
        ecs_dir = os.path.join(project_directory, "ecs")
        if not os.path.exists(ecs_dir):
            os.makedirs(ecs_dir)
            print(f"Notice: Created the {ecs_dir} directory. You can place your project-specific C++ source files and folders there.")
        
        # Create .gdignore in project/ecs/ if it doesn't exist
        gdignore_path = os.path.join(ecs_dir, ".gdignore")
        if not os.path.exists(gdignore_path):
            with open(gdignore_path, "w") as f:
                pass
            print(f"Notice: Created {gdignore_path} in 'ecs' directory to exclude it from Godot's asset scanning.")
