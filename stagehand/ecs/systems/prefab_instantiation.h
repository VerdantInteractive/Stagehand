...
inline stagehand::Registry register_prefab_instantiation_systems([](flecs::world& world)
{
        ...
        .run([&](flecs::iter& it)
    {
        ...
        if (!parameters || parameters->is_empty()) {
            godot::UtilityFunctions::push_warning(godot::String(stagehand::names::systems::PREFAB_INSTANTIATION) + ": system called without parameters. At least 'prefab' needs to be specified.");
            return;
        }

        if (!parameters->has("prefab")) {
            godot::UtilityFunctions::push_warning(godot::String(stagehand::names::systems::PREFAB_INSTANTIATION) + ": 'prefab' parameter needs to be given with the name of the prefab to instantiate.");
            return;
        }
        if ((*parameters)["prefab"].get_type() != godot::Variant::STRING)
        {
            godot::UtilityFunctions::push_warning(godot::String(stagehand::names::systems::PREFAB_INSTANTIATION) + ": 'prefab' parameter must be a String.");
            return;
        }
        ...
        if (parameters->has("components"))
        {
            ...
            {
                godot::UtilityFunctions::push_warning(godot::String(stagehand::names::systems::PREFAB_INSTANTIATION) + ": 'components' parameter must be a Dictionary.");
                return;
            }
