...
void register_multimesh_renderer(flecs::world& world, T* renderer, stagehand::entity_rendering::Renderers& renderers, int& renderer_count) {
    ...
    default:
        if constexpr (std::is_same_v<T, MultiMeshRenderer3D>) {
            if (renderer->get_draw_order() == MULTIMESH_DRAW_ORDER_Z) sort_axis = 'z';
        }
        break;
    }
    ...
    if (sort_axis != '\0') {
        switch (sort_axis) {
        case 'x':
            query.order_by<TransformType>(
                [](flecs::entity_t, const TransformType* t1, flecs::entity_t, const TransformType* t2) {
                if constexpr (std::is_same_v<T, MultiMeshRenderer2D>) {
                    ...
                    if (t1->origin.x > t2->origin.x) return 1;
                    if (t1->origin.x < t2->origin.x) return -1;
                    return 0;
                }
            });
...
template <typename T>
void bind_multimesh_renderer_methods() {
    godot::ClassDB::bind_method(godot::D_METHOD("set_prefabs_rendered", "prefabs"), static_cast<void (T::*)(const godot::PackedStringArray&)>(&T::set_prefabs_rendered));
    ...
    godot::ClassDB::add_property(T::get_class_static(), godot::PropertyInfo(godot::Variant::PACKED_STRING_ARRAY, "prefabs_rendered"), "set_prefabs_rendered", "get_prefabs_rendered");
...
void MultiMeshRenderer2D::_bind_methods() {
    bind_multimesh_renderer_methods<MultiMeshRenderer2D>();
}

void MultiMeshRenderer3D::_bind_methods() {
    bind_multimesh_renderer_methods<MultiMeshRenderer3D>();
}
