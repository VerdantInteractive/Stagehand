...
namespace stagehand {
    ...
    void ScriptsLoader::load(flecs::world& world) const
    {
        ...
        walk = [&](const godot::String& base) {
            ...
            // Check if we are in the addon root by looking for the extension file, which we'll skip to avoid loading the example Flecs scripts.
            bool is_stagehand_root = dir->file_exists("stagehand.gdextension");
            ...
            for (godot::String name = dir->get_next(); !name.is_empty(); name = dir->get_next())
            {
                ...
                if (is_dir && is_stagehand_root) { continue; }
