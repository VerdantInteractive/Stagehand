...
namespace stagehand {
    ...
    void register_vector_component_getter(const char* name)
    {
        get_component_getters()[name] = [name](const flecs::world& world, flecs::entity_t entity_id) -> godot::Variant
        {
            const T* data = nullptr;
    ...
    template <typename T, typename ElementType>
    void register_vector_component_setter(const char* name)
    {
        get_component_setters()[name] = [name](flecs::world& world, flecs::entity_t entity_id, const godot::Variant& v) {
            if (v.get_type() != godot::Variant::ARRAY) {
                godot::UtilityFunctions::push_warning(godot::String("Failed to set component '") + name + "'. Expected Array, got " + godot::Variant::get_type_name(v.get_type()));
                return;
            }
    ...
    template <typename T, typename ElementType, std::size_t N>
    void register_array_component_getter(const char* name)
    {
        get_component_getters()[name] = [name](const flecs::world& world, flecs::entity_t entity_id) -> godot::Variant
        {
            const T* data = nullptr;
    ...
    template <typename T, typename ElementType, std::size_t N>
    void register_array_component_setter(const char* name)
    {
        get_component_setters()[name] = [name](flecs::world& world, flecs::entity_t entity_id, const godot::Variant& v) {
            if (v.get_type() != godot::Variant::ARRAY) {
                godot::UtilityFunctions::push_warning(godot::String("Failed to set component '") + name + "'. Expected Array, got " + godot::Variant::get_type_name(v.get_type()));
            ...
            if (arr.size() != static_cast<int>(N)) {
                godot::UtilityFunctions::push_warning(godot::String("Failed to set component '") + name + "'. Expected array size " + godot::String::num_int64(N) + ", got " + godot::String::num_int64(arr.size()));
                return;
            }
